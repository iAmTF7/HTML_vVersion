...
const DEFAULT_EVENTS = [
  {
    ...
    banner: 'image/liveshow_mua_he.png',
...
if (storedVersion !== DATA_VERSION || !storedEvents) {
  ...
  try {
    ...
    parsedEvents.forEach(evt => {
      if (!evt.category) {
        if (evt.id === 1) evt.category = 'Pop';
        else if (evt.id === 2) evt.category = 'Acoustic';
        else if (evt.id === 3) evt.category = 'EDM/Hiphop';
        needsUpdate = true;
      }
    });
    ...
    categoryFilters.forEach(filterBtn => {
      filterBtn.addEventListener('click', function(e) {
        ...
        if (!category) {
          ...
          renderEventsList(events, '', category);
        }
      });
    });
  }
  ...
  if (currentUser) {
    ...
    if (currentUser.role === 'staff' || currentUser.role === 'admin') {
      dropdownMenu.innerHTML += `
        ...
        <li>
          <a class="dropdown-item" href="admin-checkin.html#event-management">
  ...
  if (filteredEvents.length === 0) {
    ...
    container.innerHTML = `
      <div class="col-12 text-center py-5">
        <p class="text-muted fs-4">Không tìm thấy sự kiện nào khớp với ${filterText}</p>
